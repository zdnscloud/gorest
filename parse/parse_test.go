...
func TestParse(t *testing.T) {
	...
	ut.Equal(t, ctx.Obj.GetType(), "node")
	ut.Equal(t, ctx.Obj.GetID(), "345543")
	ut.Equal(t, ctx.Obj.GetSchema().Parent, "cluster")
	ut.Equal(t, ctx.Obj.GetParent().GetID(), "123321")
	ut.Equal(t, ctx.Obj.GetSchema().Version.Group, "testing")
	ut.Equal(t, ctx.Obj.GetSchema().Version.Version, "v1")
	...
	ctx, err = Parse(nil, req, schemas)
	ut.Equal(t, err, noErr)
	ut.Equal(t, ctx.Obj.GetType(), "container")
	ut.Equal(t, ctx.Obj.GetID(), "containers123")
	ut.Equal(t, ctx.Obj.GetSchema().Parent, "pod")
	ut.Equal(t, ctx.Obj.GetParent().GetID(), "pods123")
	ut.Equal(t, ctx.Obj.GetParent().GetType(), "pod")
	ut.Equal(t, ctx.Obj.GetSchema().Version.Group, "testing")
	ut.Equal(t, ctx.Obj.GetSchema().Version.Version, "v1")
	objs := types.GetAncestors(ctx.Obj.GetParent())
	...
	ctx, err = Parse(nil, req, schemas)
	ut.Equal(t, err, noErr)
	ut.Equal(t, ctx.Obj.GetType(), "cluster")
	ut.Equal(t, ctx.Obj.GetID(), "123321")
	ut.Equal(t, ctx.Obj.GetSchema().Parent, "")
	ut.Equal(t, ctx.Obj.GetSchema().Version.Group, "testing")
	ut.Equal(t, ctx.Obj.GetSchema().Version.Version, "v1")
